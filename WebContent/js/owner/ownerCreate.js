...
function duplicateId() {
	...
	//유효성 확인
	if(!checkValueRegExp(obj, objSpan, "아이디", /^owner[a-z0-9]{1,7}/ , "owner으로 시작하고 6~12글자 사이")){
		return;
	}
...
function chkOverlap(obj) {
	...
	$.ajax({
		...
		,success : function(data) {
			if(data.resultCnt == 0) {
				$("#ownerIdSpan").removeClass("text-warning");
				$("#ownerIdSpan").addClass("text-success");
				$("#ownerIdSpan").text("'" + $("#ownerId").val() +"'" + "은 사용 가능한 아이디 입니다.");
				$("#idChk").val($("#ownerId").val());
			}else {
				$("#ownerId").focus();
				$("#ownerIdSpan").removeClass("text-success");
				$("#ownerIdSpan").addClass("text-warning");
				$("#ownerIdSpan").text("'" + $("#ownerId").val() +"'" + "은 이미 사용중인 아이디 입니다.");
				$("#idChk").val("");
			}
		}
		,error : function(xhr) {
...
function birthChk() {
	...
	//입력한 생일 정보 가져오기
	var birYear = parseInt($("#ownerBirth").val().substr(0,4));
	var age = 20;
	...
	if((currYear - birYear) +1 < age) {
		...
		$("#ownerBirthSpan").addClass("text-warning");
		$("#ownerBirthSpan").text("미성년자의 경우 보호자 연락처를 필수로 입력해야 합니다.");
		$("#ownerParentHp1").prop("readonly",false);
		$("#ownerParentHp2").prop("readonly",false);
		$("#ownerParentHp3").prop("readonly",false);
		return false;
	}	
	$("#ownerBirthSpan").text("");
	return true;
...
function save() {
	...
	$.ajax ({
		...
		,success : function(data) {
			...
			if(data.resultCnt == 1) {
				alert("저장되었습니다.");
//				location.href = "/ownerManage.do"; //목록화면으로	
				fm.method = "post"; 
				fm.action = "/ownerManage.do"; 
				fm.submit();
			}else {
...
//	}
	...
	//비밀번호확인 검사
	obj = $("#ownerPwChk");
	objSpan = $("#ownerPwChkSpan");
	if(checkValueEmpty(obj, objSpan, "비밀번호")) {
		return false;
	}
	if($("#ownerPw").val().trim() != $("#ownerPwChk").val().trim()) {
	...
	var hp2 = $("#ownerHp2");
	var hp3 = $("#ownerHp3");
	obj = $("#ownerHp1").val() + $("#ownerHp2").val() + $("#ownerHp3").val();
	objSpan = $("#ownerHpSpan");
